...
PNG_CONJUNCTION = ["conjunction.png"]

MASK_NONPARAMETRIC = [NONPARAMETRIC.sub('+tlrc.HEAD','mask_p05+tlrc.HEAD'),NONPARAMETRIC.sub('+tlrc.HEAD','mask_p01+tlrc.HEAD'),NONPARAMETRIC.sub('+tlrc.HEAD','mask_p001+tlrc.HEAD')]
...
PNG_TTEST.each do |target|
  ...
  tmp_png = Rake::FileList["*.png"].exclude(target)
  CLEAN.push(tmp_png)
  CLOBBER.push(target)
end

PNG_CONJUNCTION.each do |target|
  file target => "conjunction+tlrc.HEAD" do
    prefix = "conjunction"
    sh("DriveSuma -com viewer_cont -autorecord '#{prefix}.left.ppm' -key:d 'ctrl+left' -key 'ctrl+r'")
    sh("DriveSuma -com viewer_cont -autorecord '#{prefix}.right_medial.ppm' -key:d '[' -key 'ctrl+r' -key:d '['")
    sh("DriveSuma -com viewer_cont -autorecord '#{prefix}.bottom.ppm' -key:d 'ctrl+down' -key 'ctrl+r'")
...
MASK_NONPARAMETRIC.each do |target|
  file target => NONPARAMETRIC do
    prefix = NONPARAMETRIC.sub('+tlrc.HEAD','')
    if target.include? "p05" then
      p="p05"
    ...
    sh("3dmerge -1thresh #{critval} -prefix #{prefix}_mask_#{p} #{NONPARAMETRIC}")
  end
  CLOBBER.push(target)
end
